...
;(function (exports) {
    ...
    exports.TGeoCircle = function ($centerLat, $centerLong, $radius, $dA, $name, $description, $styleId) {
        ...
        function f_geodesist_task($lat1, $long1, $radius, $azimutDegree) {
            ...
            let sin_l1 = Math.sin(toRadian($lat1));
            let cos_S = Math.cos($S);
            let cos_l1 = Math.cos(toRadian($lat1));
            let sin_S = Math.sin($S);
            let cos_a = Math.cos(toRadian($azimutDegree));
            let sin_a = Math.sin(toRadian($azimutDegree));
             //console.log()
            ...
            let $long2 = toRadian($long1)  + Math.atan((sin_S * sin_a) / (cos_S * cos_l1 - sin_S * sin_l1 * cos_a));
