...
class MHttp extends M\Model_base
{
    ...
    public static function getJsonData_RZN($query_body) : string
    {
    ...
    public static function getJsonData_RZN_1($options = []) : string
    {
        ...
        $enterprise_name_query = urlencode($options["enterprise_name"]);
        $count = $options["count"];
        ...
        $params = [
            'param' => "draw=1&columns%5B0%5D%5Bdata%5D=col1.label&columns%5B0%5D%5Bname%5D=&
                        ...
                        $enterprise_name_query&q_no=&dt_ru_from=&dt_ru_to=&dt_ru_end_from=&dt_ru_end_to=&q_interchangeability_med_products=&q_prod_address=&q_appl_address_post=&q_prod_address_post=&q_prescription=&q_okp=&q_appl_label=",
        ];
    ...
    public static function getRegCertificateHref($DT_RowId)
    {

        $params = explode('-', $DT_RowId);
        $table_name  = $params[0];
        $int_code = $params[1];
        //Debug::print_array($params);

       // $table_name = $DT_RowId med_products_new med_products_new-9311

       //(strstr($DT_RowId, "new")) ? $table_name = "med_products_new" : $table_name = "med_products";

        //$int_code = preg_replace("/[^0-9]/", '', $register_id_uniq);

        $url = "http://roszdravnadzor.gov.ru/services/misearch?id={$int_code}&table_name={$table_name}&fancybox=true";
    ...
    public static function getEntryHref($DT_RowId)
    {
    ...
    public static function toExcel($register_id_uniq)
    {
        $filename="sheet.xls";
